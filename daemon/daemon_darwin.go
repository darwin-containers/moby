...
func (daemon *Daemon) initNetworkController(cfg *config.Config, activeSandboxes map[string]interface{}) error {
	...
	} else if err := configureNetworking(context.TODO(), daemon.netController, cfg); err != nil {
		return err
	}
...
func configureNetworking(ctx context.Context, controller *libnetwork.Controller, conf *config.Config) error {
	...
	if n, _ := controller.NetworkByName("none"); n == nil {
		if _, err := controller.NewNetwork(ctx, "null", "none", "", libnetwork.NetworkOptionPersist(true)); err != nil {
	...
	if n, _ := controller.NetworkByName("host"); n == nil {
		if _, err := controller.NewNetwork(ctx, "host", "host", "", libnetwork.NetworkOptionPersist(true)); err != nil {
